...
export class ReceiptComponent implements OnInit {
  ...
  compareSanPham_id(dshoadon) {
    ...
    for (const dmn in this.hoadons) {
      for (const dmn2 in this.hoadons[dmn].San_Pham) {
        for (const sp in this.dssanpham) {
          if (this.hoadons[dmn].San_Pham[dmn2].SanPham_id === this.dssanpham[sp]._id) {
  ...
  ChuyenTrang(number) {
    this.hoadons.splice(0, this.hoadons.length)
    for (let i = 0; i < 5; i++) {
