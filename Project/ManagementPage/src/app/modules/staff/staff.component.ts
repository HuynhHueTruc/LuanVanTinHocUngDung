...
// Biến lưu trữ từ khóa tìm kiếm
  ...
  getdsnhanvien(){
    this.NVService.getListNhanVien().subscribe((res: any) => {
      ...
      // console.log(res.nhanviens);
      // console.log(this.lengthdsnhanvien);
...
// Mãng nhân viên được check
  ...
  ThongTinGuiEmail(){
    ...
    for (let i = 0 ; i < sl; i++){
      this.ThongTinGuiEmailTaiKhoan.push({Ho_ten: this.arrTenNV[i], Mat_khau: this.arrMatKhauNV[i],
  ...
  GuiMailNhanVien(nv){
      ...
      // if (nv !== null){
      //  if (bool === 1){
      //   this.NVService.GuiEmailNhieuNhanVien(this.ThongTinGuiEmailTaiKhoan).subscribe(data =>{

      //   });
      //  } else{
      //    this.UnChecked();
      //   //  console.log('1 - Cần mở hộp thoại Email')
      //  }
      // } else{
      //   this.NhanVienChecked();
      //   this.ThongTinGuiEmail();
      //   // Tài liệu tham khảo:
      //   // https://www.it-swarm-vi.com/vi/angular/angular-2-chuyen-huong-den-mot-url-ben-ngoai-va-mo-trong-mot-tab-moi/831549701/
      //   window.open('https://mail.google.com/mail/u/0/#inbox?compose=new', '_blank');
      //   // console.log('Nhiều - Cần mở hộp thoại Email')
      // }
...
removeAccents(str) {
 ...
 SearchByKeyWord(){
  this.dsnhanvien = this.dsnhanviensearch;
