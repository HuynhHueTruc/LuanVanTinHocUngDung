...
export class SupplierComponent implements OnInit {
  ...
  open_update(content_update, nhacungcapUpdate) {
    ...
    for (const dm in this.subdanhmuc) {
      for (const j in this.nhacungcap.Danh_muc_cung_cap) {
       if (this.nhacungcap.Danh_muc_cung_cap[j].DMN_id === this.subdanhmuc[dm].DMN_id){
         this.danhmuctmp.push(this.subdanhmuc[dm])
       }
      }
    }
    ...
    if (ncc === null) {
      ...
      for (const email in this.ThongTinGuiEmailTaiKhoan) {
        if (this.ThongTinGuiEmailTaiKhoan.hasOwnProperty(email)) {
          this.dsNhaCungCapNhanMail.push({
  ...
  open_supplier_plus(content_info_supplier_plus) {
    this.modalService.open(content_info_supplier_plus, {
